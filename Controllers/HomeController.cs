...
using static StockWebSample.Models.HomeModel;

namespace StockWebSample.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult GetRealtimePrice(GetRealtimePriceIn inModel)
        {
            ...
            for (int i = 0; i < jsonPrice.msgArray.Count; i++)
            {
                ...
                // 公司簡稱
                string name = jsonPrice.msgArray[i].n;
                ...
                sbRealPrice.Append("代碼: " + code + "|" + name + " 收盤價: " + close + " 最低委賣價: " + ask + " 最高委買價: " + bid + "<br>");
            }
        ...
        public ActionResult GetMonthPrice(GetMonthPriceIn inModel)
        {
            ...
            {
                outModel.gridList = new List<StockPriceRow>();
                string[] lineStrs = downloadedData.Split(new string[] { "\r\n", "\r", "\n" }, StringSplitOptions.None);
                for (int i = 0; i < lineStrs.Length; i++)
                {
                    string strline = lineStrs[i];
                    if (i == 0 || i == 1 || strline.Trim().Length == 0 || strline.IndexOf(',') == -1)
                    {
                        continue;
                    }
                    // 排除非價格部份
                    ...
                    //    continue;
                    //}
                    //if (strline.IndexOf("說明") > -1 || strline.IndexOf("符號") > -1 || strline.IndexOf("統計") > -1 || strline.IndexOf("ETF") > -1 || strline.IndexOf("權證") > -1)
                    //{
                    //    continue;
                    //}
